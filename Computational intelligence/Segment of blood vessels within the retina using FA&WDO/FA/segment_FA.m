...
nPop = 15;            % Number of Fireflies (Swarm Size)
...
segmented=(reshape(FAlbl(:,1),size(I)));
segmented(segmented == 2) = 0;
segmented(segmented == 1) = 1;
mask(mask==255)=1;
%mask(mask==1)=0;
segmented=double(segmented).*double(mask);
segmented=imfilter(segmented,[5,5]);
% Plot Results 
figure;
%subplot(1,2,1);
%imshow(img);title('Original');
%subplot(1,2,2);
imshow(segmented,[]);title('Segmented Image FA');
